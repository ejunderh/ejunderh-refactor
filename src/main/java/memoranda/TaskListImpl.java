...
public class TaskListImpl implements ITaskList {
    ...
    public CalendarDate getEarliestStartDateFromSubTasks(ITask task) {
        CalendarDate date = task.getStartDate();
        if (hasSubTasks(task.getID())) {
	        Collection subTasks = getAllSubTasks(task.getID());
	        for (Iterator iter = subTasks.iterator(); iter.hasNext();) {
	        	ITask e = (ITask) iter.next();
	        	CalendarDate earlyDate = getEarliestStartDateFromSubTasks(e);
	        	if(earlyDate.before(date)) {
	        	    date = earlyDate;
	        	}
	        }
